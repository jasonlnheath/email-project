...
/// OpenAI-compatible API provider (works with OpenAI, local models, etc.)
actor OpenAIProvider: LLMProvider {
    ...
    func chat(messages: [[String: String]], systemPrompt: String? = nil) async throws -> String {
        ...
        if let systemPrompt = systemPrompt {
            // OpenAI uses a different format for system prompts
        ...
        do {
            ...
            throw LLMError.apiError(statusCode: -1, body: error.localizedDescription)
        }
    }

    /// Summarize a single email
    func summarizeEmail(subject: String, sender: String, body: String) async throws -> SummarizeResult {
        print("\n🤖 === OpenAI summarizeEmail() called ===")
        ...
        print("📤 Sending prompt to OpenAI, length: \(messages[0]["content"]?.count ?? 0)")
        print("📤 Prompt preview (first 500 chars): \(String((messages[0]["content"] ?? "").prefix(500)))")

        let response = try await chat(messages: messages)

        print("📥 OpenAI response length: \(response.count)")
        print("📥 OpenAI response preview: \(String(response.prefix(500)))")
