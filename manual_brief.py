#!/usr/bin/env python3
"""Manual brief - direct API calls, no subprocess spawning."""
import json
import time
...
data = http_get(f"{SERVER_URL}/api/stats", timeout=5)
if data and "total_emails" in data:
    log(f"Server running ({data['total_emails']} emails)")
else:
...
result = http_get(f"{SERVER_URL}/api/reload", timeout=30)
if result:
...
for i in range(90):
    stats = http_get(f"{SERVER_URL}/api/stats", timeout=5)
    if not stats:
        time.sleep(2)
        continue
...
# Get final stats
final_stats = http_get(f"{SERVER_URL}/api/stats", timeout=5) or {}
